...
const normalizeStatus = (value) => {
    ...
    if (/iptal/i.test(text) || /iptal/i.test(text)) return '\İptal Edildi';
    return 'Hazırlanıyor';
};
...
const Orders = () => {
    ...
    const filteredCustomers = useMemo(() => {
        ...
        return subscribers.filter((subscriber) =>
            ...
            (subscriber.phone || '').includes(customerSearch) ||
    ...
    const activeMenuOrder = paginatedOrders.find((order) => order.id === activeMenu) || null;
    ...
    const handleCreateOrder = async (event) => {
        ...
        await addOrder({
            ...
            product: items.map((item) => `${item.quantity}x ${item.name}`).join(', '),
            items,
            quantity,
            amount,
        ...
        setNewOrder({ customerId: '', items: [{ productId: '', quantity: 1 }], courier: '-', paymentMethod: 'Nakit' });
        setCustomerSearch('');
    };
    ...
    const handleUpdateOrder = async (event) => {
        ...
        await updateOrder(selectedOrder.id, {
            ...
            amount: Number(editOrderData.amount || 0),
    ...
    const handleCreateInvoice = async (orderId) => {
        await updateOrder(orderId, { hasInvoice: true });
        useStore.getState().addNotification(`Fatura oluşturuldu! (#${String(orderId).slice(-6).toUpperCase()})`, 'success');
    };
    ...
    const addEditItem = () => {
        setEditOrderData((previous) => ({
            ...previous,
            items: [...previous.items, { productId: '', name: '', quantity: 1, price: 0 }],
        }));
    };
    ...
    const updateEditItem = (index, field, value) => {
        ...
        items[index] = { ...items[index], [field]: value };
        const amount = items.reduce((total, item) => total + Number(item.price || 0) * Number(item.quantity || 0), 0);
        setEditOrderData((previous) => ({ ...previous, items, amount }));
    };
    ...
    return (
        <div className="p-4 md:p-8 bg-slate-50 min-h-screen selection:bg-brand-primary/10 w-full overflow-hidden">
            ...
            <div className="lg:hidden space-y-4 mb-8">{paginatedOrders.map((order) => { const statusInfo = STATUSES.find((status) => status.id === order.status) || STATUSES[0]; return <div key={order.id} className="premium-card p-6 relative"><div className="flex justify-between items-start mb-4"><div className="flex items-center gap-4 min-w-0"><div className="w-12 h-12 shrink-0 bg-slate-900 text-white rounded-2xl flex items-center justify-center font-black text-xl">{order.customer ? order.customer.charAt(0) : '?'}</div><div className="min-w-0"><h3 className="font-bold text-slate-900 truncate">{order.customer}</h3><div className="flex flex-wrap items-center gap-2 mt-1"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">#{order.orderNumber || String(order.id).slice(-6).toUpperCase()}</p><span className={`text-[9px] font-black px-2 py-0.5 rounded-md border ${order.paymentMethod === 'Veresiye' ? 'bg-amber-50 text-amber-600 border-amber-100' : order.paymentMethod === 'Nakit' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>{(order.paymentMethod || 'Nakit').toUpperCase()}</span></div></div></div><button onClick={() => setActiveMenu(activeMenu === order.id ? null : order.id)} className="p-2 text-slate-400 shrink-0"><MoreHorizontal size={20} /></button></div><div className="space-y-3"><div className="flex items-center gap-3 text-xs text-slate-600 font-bold min-w-0"><Package size={16} className="text-slate-300 shrink-0" /> <span className="truncate">{order.product}</span> <span className="text-brand-primary shrink-0">x{order.quantity}</span></div><div className="flex justify-between items-center pt-3 border-t border-slate-50 gap-3"><span className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border ${statusInfo.bg} ${statusInfo.color} ${statusInfo.border}`}>{statusInfo.label}</span><div className="text-lg font-black text-slate-900 font-display shrink-0">{formatMoney(order.amount)}</div></div></div></div>; })}</div>
                    ...
                    <div className="absolute inset-x-0 bottom-0 rounded-t-[2rem] bg-slate-950 text-white shadow-2xl border-t border-white/10 p-4 pb-[calc(env(safe-area-inset-bottom,0px)+1rem)] animate-in slide-in-from-bottom duration-200">
                        ...
                        <div className="space-y-2">
                            ...
                            {!activeMenuOrder.hasInvoice && activeMenuOrder.status === 'Tamamlandı' && (
                                <button onClick={() => { handleCreateInvoice(activeMenuOrder.id); setActiveMenu(null); }} className="w-full text-left px-4 py-4 rounded-2xl text-xs font-black text-slate-100 bg-white/5 hover:bg-emerald-500/15 transition-all flex items-center gap-3">
                            ...
                            <button onClick={() => handleDeleteOrder(activeMenuOrder.id)} className="w-full text-left px-4 py-4 rounded-2xl text-xs font-black text-rose-100 bg-rose-500/15 hover:bg-rose-500 transition-all flex items-center gap-3 border border-rose-500/20">
