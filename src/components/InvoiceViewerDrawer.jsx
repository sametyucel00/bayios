...
import { XCircle, Printer, Download, MapPin, Phone, Building2, CheckCircle2 } from 'lucide-react';
...
const InvoiceViewerDrawer = ({ isOpen, onClose, order }) => {
    ...
    const invoiceDate = new Date(order.timestamp).toLocaleDateString('tr-TR');

    // Simulate KDV (Tax)
    const kdvRate = 0.20; // 20%
                            ...
                            <table className="w-full text-left min-w-[500px]">
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {order.items?.map((item, idx) => {
                                        // Fake breakdown if exact product details are missing
                                        const unitPrice = parseFloat(order.amount) / parseInt(order.quantity || 1);
                                        return (
                                            <tr key={idx}>
                                                <td className="py-4 text-sm font-bold text-slate-800">{order.product}</td>
                                                <td className="py-4 text-sm font-bold text-slate-600 text-center">{order.quantity}</td>
                                                <td className="py-4 text-sm font-bold text-slate-600 text-right">₺{unitPrice.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</td>
                                                <td className="py-4 text-sm font-black text-slate-900 text-right">₺{totalAmount.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</td>
                                            </tr>
                                        );
                                    }) || (
                                            <tr>
                                                <td className="py-4 text-sm font-bold text-slate-800">{order.product}</td>
                                                <td className="py-4 text-sm font-bold text-slate-600 text-center">{order.quantity}</td>
                                                <td className="py-4 text-sm font-bold text-slate-600 text-right">₺{(totalAmount / (order.quantity || 1)).toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</td>
                                                <td className="py-4 text-sm font-black text-slate-900 text-right">₺{totalAmount.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</td>
                                            </tr>
                                        )}
                                </tbody>
                        ...
                        <div className="flex justify-end border-t border-slate-100 pt-8">
                            <div className="w-full sm:w-64 space-y-3">
                                ...
                                <div className="flex justify-between items-center pt-4 border-t-2 border-slate-800">
                                    ...
                                    <span className="text-xl font-black text-slate-900 font-display">₺{totalAmount.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</span>
                                </div>
                            </div>
                        </div>
