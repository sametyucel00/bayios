...
const SyncStatus = () => {
    ...
    return (
        <div className="px-5 py-2 mt-1">
            <div className={`px-3 py-2.5 rounded-xl flex items-center justify-between gap-2 ${syncError ? 'bg-rose-500/10 border border-rose-500/20' : 'bg-white/5 border border-white/5'}`}>
...
const Sidebar = ({ currentView, setCurrentView, isOpen, onClose, onLogout, user }) => {
    ...
    fixed inset-y-0 left-0 z-50 w-full md:w-80 bg-slate-900/98 backdrop-blur-2xl text-white transform transition-all duration-500 ease-in-out border-r border-white/5
    ...
    return (
        <>
            ...
            <aside className={sidebarClasses}>
                <div className="p-10 border-b border-white/10 flex justify-between items-center group/logo">
                    <div>
                        <div className="flex items-center gap-4 mb-2 transition-transform duration-500 group-hover/logo:translate-x-1">
                            <div className="w-10 h-10 rounded-2xl overflow-hidden shadow-xl shadow-brand-primary/20 group-hover/logo:rotate-12 transition-transform duration-500 bg-white p-1">
                        ...
                        <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em] pl-1 opacity-80">
                    ...
                    {menuItems.map((item) => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl relative ${isActive
                            ...
                            >
                                <Icon size={22} className={`${isActive ? 'text-white' : 'text-slate-500 group-hover:text-brand-primary'}`} />
                                <span className={`text-[13px] tracking-wide ${isActive ? 'font-black' : ''}`}>{item.label}</span>
                            </button>
                        );
                    })}
                </nav>

                <div className="p-6 border-t border-white/5 bg-black/20 space-y-2">
                    <button
                        onClick={() => {
                            setCurrentView('settings');
                            if (window.innerWidth < 768) onClose();
                        }}
                        className={`w-full flex items-center gap-4 px-5 py-3.5 rounded-2xl transition-all duration-300 ${currentView === 'settings'
                    ...
                    <button
                        onClick={onLogout}
                        className="w-full flex items-center gap-4 px-5 py-3.5 text-rose-400 hover:bg-rose-500/10 hover:text-rose-300 rounded-2xl transition-all group font-black"
                    >
